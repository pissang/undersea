...
void main()
{
    ...
    float w;
    for (int _idx_ = 0; _idx_ < 9; _idx_++) {{
        coord = vec2(clamp(v_Texcoord.x + (float(_idx_) - 4.0) * off, 0.0, 1.0), v_Texcoord.y);
        // Use depth in bilateral filter instead normal.
        // Use normal will cause jitter in the corner.
        w = gaussianKernel[_idx_] * max(0.0, 1.0 - edgeSharpness * 1000.0 * abs(centerDepth - texture2D(depthTex, coord).r));
        weightAll += w;
...
void main()
{
    ...
    float w;
    for (int _idx_ = 0; _idx_ < 9; _idx_++) {{
        coord = vec2(v_Texcoord.x, clamp(v_Texcoord.y + (float(_idx_) - 4.0) * off, 0.0, 1.0));
        w = gaussianKernel[_idx_] * max(0.0, 1.0 - edgeSharpness * 1000.0 * abs(centerDepth - texture2D(depthTex, coord).r));
        weightAll += w;
        sum += texture2D(colorTex, coord) * w;
    }}
